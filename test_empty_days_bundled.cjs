...
function buildEthiopicDate(year, month, day) {
  ...
  return (0, import_dayjs.default)(new Date(greg[0], greg[1] - 1, greg[2])).toCalendarSystem("ethiopic");
}
function evaluateEmptyDays(year, month, t) {
  let date = buildEthiopicDate(year, month, 1);
  let firstDay = date.date(1).day();
  let emptyProper = (firstDay - t + 7) % 7;
  let refOld = 7 - t;
  let emptyOld = date.date(refOld).day();
  let refClamped = Math.min(7 - t, date.daysInMonth());
  let emptyClamped = date.date(refClamped).day();
  console.log(`t=${t} Y=${year} M=${month}: Day 1 is ${firstDay}. Proper: ${emptyProper}, Old: ${emptyOld}, Clamped: ${emptyClamped}`);
}
console.log("=== Nehase ===");
evaluateEmptyDays(2018, 10, 1);
console.log("=== Pagume ===");
evaluateEmptyDays(2018, 11, 1);
evaluateEmptyDays(2018, 12, 1);
