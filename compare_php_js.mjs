...
let date = dayjs("2024-04-10");
let eth = date.toCalendarSystem("ethiopic");
console.log(`JS Ethiopic for 2024-04-10: Y=${eth.year()} M=${eth.month()+1} D=${eth.date()}`);

let now = dayjs();
let ethNow = now.toCalendarSystem("ethiopic");
let phpNowOut = require('child_process').execSync(`php -r "require 'vendor/autoload.php'; \\$e = new \\\\Mammesat\\\\FilamentEthiopicDatePicker\\\\Services\\\\EthiopicCalendar(); echo json_encode(\\$e->getEthiopianDate(${now.year()}, ${now.month()+1}, ${now.date()}));"`).toString();
let phpNow = JSON.parse(phpNowOut);
