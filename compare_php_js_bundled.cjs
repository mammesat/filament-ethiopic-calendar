  ...
  "node_modules/@calidy/dayjs-calendarsystems/dayjs-calendarsystems.cjs.min.js"(exports2, module2) {
    ...
    module2.exports = (t2, e2, r2) => {
      ...
      e2.prototype.init = function(t3) {
        ...
        var l2 = 12;
        switch ("hebrew" == this.$C && (l2 = this.isLeapYear() ? 13 : 12), "ethiopic" == this.$C && (l2 = 13), n3) {
          case "year":
            return r3 ? i3(1, 0) : i3(0, 0, this.$y + 1);
          case "month":
            return r3 ? i3(1, this.$M) : i3(0, (this.$M + 1) % l2, this.$y + parseInt((this.$M + 1) / l2, 10));
          case "week": {
      ...
      e2.prototype.$set = function(t3, e3) {
        ...
        const { $d: r3, $u: n3, $C: o3, $y: s3, $M: i3, $D: a2, $H: y2, $m: u2, $s: $2, $ms: l2 } = this, d2 = n3 ? "UTC" : "", m2 = (t4, e4) => r3[`set${d2}${t4}`](e4), f = (t4, e4, r4 = s3, n4 = y2, i4 = u2, a3 = $2, c2 = l2) => {
      ...
      r2.locale = function(t3, e3, r3) {
        return e3 = l(t3, e3, n2), m.bind(this)(t3, e3, r3);
...
function toGregorian(t) {
  ...
  var r = (f - 1) * 30 + m;
  r <= 37 && i <= 1575 ? (r += 28, l[0] = 31) : r += $ - 1, i - 1 % 4 === 3 && (r += 1);
  for (var s = 0, e = void 0, n = 0; n < l.length; n++)
    if (r <= l[n]) {
      s = n, e = r;
      break;
    } else
...
var date = (0, import_dayjs.default)("2024-04-10");
var eth = date.toCalendarSystem("ethiopic");
console.log(`JS Ethiopic for 2024-04-10: Y=${eth.year()} M=${eth.month() + 1} D=${eth.date()}`);
var now = (0, import_dayjs.default)();
var ethNow = now.toCalendarSystem("ethiopic");
var phpNowOut = require("child_process").execSync(`php -r "require 'vendor/autoload.php'; \\$e = new \\\\Mammesat\\\\FilamentEthiopicDatePicker\\\\Services\\\\EthiopicCalendar(); echo json_encode(\\$e->getEthiopianDate(${now.year()}, ${now.month() + 1}, ${now.date()}));"`).toString();
var phpNow = JSON.parse(phpNowOut);
console.log(`Today Gregorian: ${now.format("YYYY-MM-DD")}`);
console.log(`JS Today Ethiopic: Y=${ethNow.year()} M=${ethNow.month() + 1} D=${ethNow.date()}`);
console.log(`PHP Today Ethiopic: Y=${phpNow[0]} M=${phpNow[1]} D=${phpNow[2]}`);
